...
namespace Libptx.Edsl.TextGenerators
{
    internal static class EnumGenerator
    {
        public static void DoGenerate()
        {
            ...
            w.WriteLine("{");
            w.Indent++;
            ops.ForEach(op => w.WriteLine(op.Name + ","));
            w.Indent--;
            w.WriteLine("}");
            w.Indent--;
            w.WriteLine("}");
            ...
            w.WriteLine("{");
            w.Indent++;
            w.WriteLine("public abstract ptxoptype optype { get; }");
            w.Indent--;
            w.WriteLine("}");
            w.Indent--;
            w.WriteLine("}");
            ...
            ops.ForEach(op =>
            {
                ...
                w.WriteLine("{");
                w.Indent++;
                w.WriteLine("public override ptxoptype optype { get { return ptxoptype." + op.Name + "; } }");
                w.Indent--;
                w.WriteLine("}");
                w.Indent--;
                w.WriteLine("}");
